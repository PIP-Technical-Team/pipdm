...
db_create_metadata_table <- function(pfw_table, lcu_table) {
  ...
  # PFW columns
  pfw_table <-
    pfw_table[, c('region_code', 'country_code', 'survey_coverage', 'survey_acronym',
  ...
  # LCU columns
  lcu_table <-
    lcu_table[, c('survey_id', 'country_code', 'surveyid_year', 'survey_acronym',
                  'survey_year', 'welfare_type', 'distribution_type', 'gd_type',
                  'cpi_data_level', 'ppp_data_level', 'gdp_data_level', 'pce_data_level'
    )]
  ...
  # Order columns
  dt %>% data.table::setcolorder(c('survey_id', 'region_code', 'country_code' ,
