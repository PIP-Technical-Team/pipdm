test_that('select_proxy() works as expected', {
  ...
  df <- data.frame(svy_pce = 100, svy_gdp = 200)
  ...
  df <- data.frame(svy_pce = c(100, NA), svy_gdp = c(200, 210))
  out <- select_proxy(df, region_code = 'ECA', ref_pce = 105, ref_gdp = 205)
  ...
  df <- data.frame(svy_pce = c(100, 105), svy_gdp = c(200, 205))
  out <- select_proxy(df, region_code = 'SSA', ref_pce = 105, ref_gdp = NA)
...
test_that('check_inputs_select_proxy() catches errors correctly', {
  # Too many rows
  df <- data.frame(svy_pce = c(100, 105, 110), svy_gdp = c(200, 205, 210))
  expect_error(check_inputs_select_proxy(df, 'ECA'))
  # svy_gdp missing
  df <- data.frame(svy_pce = c(100, 105))
  expect_error(check_inputs_select_proxy(df, 'ECA'))
  # svy_pce missing
  df <- data.frame(svy_gdp = c(100, 105))
  expect_error(check_inputs_select_proxy(df, 'ECA'))
